...
- Limit 开启限价单功能：
  - 勾选后，将开启 MaxO、VA、VB 功能。如不勾选，是纯市价单模式，MaxO、VA、VB 参数均不生效。
  - 勾选后，仍然可以通过不填 VA、VB 来使用市价单。
- MaxO：单边未成交的最大挂单数量，单位是订单。
  - **不填，就是没有限制**。
  - 有可能会因为下单请求延迟，或者未成交订单没有及时更新等问题导致未成交订单数超过这个数字。
- VA，VB：价距。即委托价与买一卖一价的价差，单位是百分比。：
  - **不填，下市价单，其余情况下限价单**。
