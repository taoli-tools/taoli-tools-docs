---
title: Backpack 设置 - Taoli Tools 使用手册
description: 配置 Backpack 交易所 API,支持现货、合约和杠杆交易,35% 返佣
head:
  - - meta
    - name: keywords
      content: Backpack,CEX,现货交易,永续合约,Taoli Tools,API 配置
---

# Backpack

返佣：35%

邀请码：`okgkcxvl`

邀请链接：[https://backpack.exchange/refer/okgkcxvl](https://backpack.exchange/refer/okgkcxvl)

> [!WARNING]
> 因 Backpack API 限制，必须解除浏览器的跨域限制才可以使用，教程在 [解除浏览器跨域限制](../disable-browser-cors/)

1. 打开「API 密钥」页面 [https://backpack.exchange/settings/api-keys](https://backpack.exchange/settings/api-keys)
2. 点击「新建 API Key」，**不要**选「只读」。

   ![image.png](backpack/image.png)

3. 建议使用 Limit 模式下单，因为 Backpack 很多交易对都不支持 Market Order。
